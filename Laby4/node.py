...
class Node:
    ...
    def countGiniSide(self, side, side_pos, side_neg):
        return 1 - pow(side_pos/side, 2) - pow(side_neg/side, 2)
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split in possible_splits:
            left_tree = y[:split + 1]
            right_tree = y[split + 1:]

            left_pos = np.sum(left_tree == 1)
            left_neg = np.sum(left_tree == 0)
            right_pos = np.sum(right_tree == 1)
            right_neg = np.sum(right_tree == 0)

            left = left_pos + left_neg
            right = right_pos + right_neg
            sum_left_right = left + right
            gini_left = self.countGiniSide(left, left_pos, left_neg)
            gini_right = self.countGiniSide(right, right_pos, right_neg)

            gini_gain = 1 - left/sum_left_right*gini_left - right/sum_left_right*gini_right
