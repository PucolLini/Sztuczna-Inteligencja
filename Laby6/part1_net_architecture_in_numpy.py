...
def zad1_single_neuron(student_id):
    ...
    # TODO: ustawienie właściwych wag (0.5 point) - metoda prob i bledow
    model.W[:, 0] = [1,2] #w1,w2,b1=1,2,-2.5
    model.b[:] = [-2.5]
...
def zad2_two_layer_net(student_id):
    ...
    model.hidden_layer.W[:, 0] = [-0.3, 1]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [0.3, -1]      # wagi neuronu h2
    model.hidden_layer.b[:] = [-0.27, 0.5]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [1, 1]      # wagi neuronu wyjściowego
    model.output_layer.b[:] = [-0.6]         # bias neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 193592         # Twój numer indeksu, np. 102247
